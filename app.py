...
# НОВОЕ: Динамическая настройка базы данных: PostgreSQL (для продакшена) или SQLite (для локального dev)
...
def initialize_db_data():
    ...
    Эта функция будет выполняться только в том случае, если таблицы еще не существуют 
    (например, при первом запуске или при использовании SQLite).
    """
    with app.app_context():
        engine = db.engine
        
        # Проверяем, существует ли таблица Poem. Если нет, создаем все таблицы и наполняем начальными данными.
        # Это предотвратит сброс данных на внешней PostgreSQL базе.
